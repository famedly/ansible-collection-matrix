...
from ansible_collections.famedly.matrix.tests.unit.mock_nio.utils.RoomSimulator import RoomEvents
...
class RoomBan:
    async def room_ban(self,
        ...
        self.room_sim.m_room_member(room_id=room_id, membership="ban", sender=self.user, state_key=user_id)
...
class RoomGetState:
    async def room_get_state(self,
        ...
        return RoomGetStateResponse(events=self.room_sim.get_events(room_id), room_id=room_id)
...
class RoomGetStateEvent:
    async def room_get_state_event(self,
        ...
        return RoomGetStateEventResponse(content=event['content'], event_type=event_type, state_key=state_key,
                                         room_id=room_id)
...
class RoomInvite:
    async def room_invite(self,
        ...
        self.room_sim.m_room_member(room_id=room_id, membership="join", sender=self.user, state_key=user_id)
...
class RoomKick:
    async def room_kick(self,
        ...
        self.room_sim.m_room_member(room_id=room_id, membership="leave", sender=self.user, state_key=user_id)
...
class RoomResolveAlias:
    async def room_resolve_alias(self,
        ...
        if room_id is not None:
            return RoomResolveAliasResponse(room_id=room_id, room_alias=room_alias, servers=[])
        else:
